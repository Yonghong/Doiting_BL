# 1 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
# 1 "/home/drxiang/programming/Doiting_BL/examples/WIFI_CLIENT/build_out/hal_drv//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
# 30 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
...
# 31 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 32 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 33 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 34 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 35 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 36 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 38 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 40 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 50 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c" 2
...
# 124 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
static void update_mac_config_with_order(const void *fdt, int offset1, const char *order)
{
    ...
    for (i = 0; i < (3) && i < len; i++) {
        switch (order[i]) {
            ...
            default:
            {
                do { if (!(0)) { do { ; bl_printk("[%10u][%s: %s:%4d] " "ASSERT: %s:%d\r\n", (xPortIsInsideInterrupt())?(xTaskGetTickCountFromISR()):(xTaskGetTickCount()), "\x1b[35mASSERT\x1b[0m", "hal_board.c", 169, "hal_board.c", 169); ; } while(0==1); while (1) { } } } while (0);
            }
        }
    }
break_scan:
...
static int update_xtal_config_get_mac_from_dtb(const void *fdt, int offset1, uint32_t capcode[5])
{
    ...
    if (5*4 == lentmp) {
         ...
         ;
        capcode[0] = ((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*0)));
        capcode[1] = ((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*1)));
        capcode[2] = ((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*2)));
        capcode[3] = ((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*3)));
        capcode[4] = ((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*4)));
    } else {
        ;
        return -1;
    }
...
static void update_xtal_config_with_order(const void *fdt, int offset1, const char *order)
{
    ...
    for (i = 0; i < (2) && i < len; i++) {
        switch (order[i]) {
            ...
            default:
            {
                do { if (!(0)) { do { ; bl_printk("[%10u][%s: %s:%4d] " "ASSERT: %s:%d\r\n", (xPortIsInsideInterrupt())?(xTaskGetTickCountFromISR()):(xTaskGetTickCount()), "\x1b[35mASSERT\x1b[0m", "hal_board.c", 290, "hal_board.c", 290); ; } while(0==1); while (1) { } } } while (0);
            }
        }
    }
break_scan:
...
static int update_poweroffset_config_get_mac_from_dtb(const void *fdt, int offset1, int8_t poweroffset[14])
{
    ...
    if (14*4 == lentmp) {
        for (i = 0; i < 14; i++) {
            poweroffset[i] = ((uint32_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*i)));
        }
        ;
        ;
        for (i = 0; i < 14; i++) {
...
static void update_poweroffset_config_with_order(const void *fdt, int offset1, const char *order)
{
    ...
    for (i = 0; i < (2) && i < len; i++) {
        switch (order[i]) {
            ...
            default:
            {
                do { if (!(0)) { do { ; bl_printk("[%10u][%s: %s:%4d] " "ASSERT: %s:%d\r\n", (xPortIsInsideInterrupt())?(xTaskGetTickCountFromISR()):(xTaskGetTickCount()), "\x1b[35mASSERT\x1b[0m", "hal_board.c", 415, "hal_board.c", 415); ; } while(0==1); while (1) { } } } while (0);
            }
        }
    }
break_scan:
...
static int update_sta_field(const void *fdt, int wifi_offset, const char *name)
{
    ...
    if (offset1 > 0) {
        ...
        if (addr_prop) {
            ...
            auto_connect_enable = ((uint32_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 0)));
        } else {
...
static int update_ap_field(const void *fdt, int wifi_offset, const char *name)
{
    ...
    if (offset1 > 0)
    {
        ...
        if (addr_prop) {
            ...
            ap_channel = ((uint32_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 0)));
        } else {
            ;
        }
...
static int hal_board_load_fdt_info(const void *dtb)
{
    ...
    if (offset1 > 0) {
        ...
        ((void)(lo_fcal_div));
        ((void)(channel_div_table));
        ((void)(channel_cnt_table));
        ...
        if (15*4 == lentmp) {
            for (i = 0; i < 15; i++) {
                channel_div_table[i] = ((uint32_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*i)));
            }
            ;
            ;
        } else {
        ...
        if (14*4 == lentmp) {
            for (i = 0; i < 14; i++) {
                channel_cnt_table[i] = ((uint16_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*i)));
            }
            ;
            ;
        } else {
        ...
        if (4 == lentmp) {
            lo_fcal_div = ((uint16_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*0)));
            ;
        } else {
            ;
        }
    ...
    if (offset1 > 0) {
        ...
        if (4 == lentmp) {
            ...
            bl_wifi_country_code_set(((uint8_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*0))));
        } else {
            ;
        }
    }
    ...
    if (offset1 > 0)
    {
        ...
        if (4*4 == lentmp) {
            for (i = 0; i < 4; i++) {
                pwr_table[i] = ((uint32_t)fdt32_to_cpu(*(uint32_t *)((uint8_t *)addr_prop + 4*i)));
            }
...
# 674 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
             ;
...
# 694 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
             ;
...
# 842 "/home/drxiang/programming/Doiting_BL/bl_iot_sdk/components/hal_drv/bl602_hal/hal_board.c"
